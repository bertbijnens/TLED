...
app_driver_handle_t app_driver_light_init(void)
{
    ...
    // Buffer size: use max of configured LEDs or 100 to clear leftover data on
    // strips where the user has reduced the LED count since last boot.
    ...
    if (config->chipset == CHIPSET_WS2805) {
        ...
        // Over-provision to strip_buffer_size (same as other chipsets) so that
        // reducing the LED count sends zeros to the now-unused ICs, turning them off.
